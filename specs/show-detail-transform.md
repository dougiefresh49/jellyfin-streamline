...
3. **Render** our own DOM into a container `#streamline-detail` appended to the page; hide Jellyfin's native detail content while ours is shown. Compose per gpt layout:
...
5. **Cleanup on navigate-away:** remove `#streamline-detail`, unhide native content, tear down observers/listeners (no leaks, no duplicate containers).
