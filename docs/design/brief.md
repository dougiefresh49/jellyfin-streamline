...
You are a senior mobile UI/UX designer. Produce a visual redesign CONCEPT as a self-contained HTML mockup for a self-hosted **Jellyfin** media server's web UI. It is viewed inside the Jellyfin mobile app (an Android/iOS web wrapper) on phones and tablets. Today it looks generic and dated ("like Fandango / a database"). Make it feel like a **modern streaming app — Netflix / Disney+**: cinematic, poster-forward, minimal chrome, fast to the content.
...
6. **Remove the per-episode heart button** on each episode row (clutters the row). Keep a play affordance, maybe a subtle "watched" tick, and overflow.
...
- `ref-netflix-kids-home-1.png` / `ref-netflix-kids-home-2.png`: Netflix kids home — big colorful rounded cards in horizontal rows ("Favorites", "Only on Netflix", "Continue Watching for Emma & Eli"), playful, bottom nav (Home / Search / My Netflix). Good reference for streaming-app warmth and horizontal rows.
