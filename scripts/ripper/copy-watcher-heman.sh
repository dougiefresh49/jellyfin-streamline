...
while true; do
  ...
  for f in ${files[@]+"${files[@]}"}; do
    ...
    if ! rsync -t "$f" "$dest"; then
      log "rsync FAILED: $rel"; slack "🚨 he-man copy watcher: rsync failed for $rel"; sleep 30; continue
    fi
    src_size=$(stat -f%z "$f"); dst_size=$(stat -f%z "$dest")
    if [ "$src_size" != "$dst_size" ]; then
      log "size mismatch: $rel ($src_size vs $dst_size)"; slack "🚨 he-man copy watcher: size mismatch for $rel"; continue
    fi

    epdir="$LAB_SHOW/$code"
    mkdir -p "$epdir"
    if extract_dialogue "$f" "$epdir/center.flac"; then
      grep -qx "HeMan-1983/$code" "$QUEUE" || echo "HeMan-1983/$code" >> "$QUEUE"
    else
