#!/bin/bash
# Ships finished TMNT (2003) episodes from ready-to-copy/ to the Seagate, then
# extracts audio for voice-lab and queues the episode for Demucs.
# 5.1 discs give up the center channel; stereo discs go to Demucs untouched
# (it was trained on stereo mixes — better separation than a mono downmix).
# Seasons 1-3 were shipped before the voice lab existed and still need a
# backfill pass; from S4 on, episodes enter the queue as they ship.
...
while true; do
  ...
  for f in ${files[@]+"${files[@]}"}; do
    ...
    if ! rsync -t "$f" "$dest"; then
      log "rsync FAILED: $rel"; slack "🚨 tmnt2003 copy watcher: rsync failed for $rel"; sleep 30; continue
    fi
    src_size=$(stat -f%z "$f"); dst_size=$(stat -f%z "$dest")
    if [ "$src_size" != "$dst_size" ]; then
      log "size mismatch: $rel ($src_size vs $dst_size)"; slack "🚨 tmnt2003 copy watcher: size mismatch for $rel"; continue
    fi

    epdir="$LAB_SHOW/$code"
    mkdir -p "$epdir"
    if extract_dialogue "$f" "$epdir/center.flac"; then
      grep -qx "TMNT-2003/$code" "$QUEUE" || echo "TMNT-2003/$code" >> "$QUEUE"
    else
